...
function botStatePath(): string {
    return path.resolve(__dirname, "..", "src/data/predictive-arb-state.json");
}
...
function loadBotStateFile(): BotStateFile {
    ...
    try {
        ...
        logger.error(`Failed to read predictive-arb-state.json for pnl: ${e instanceof Error ? e.message : String(e)}`);
        return {};
    }
}
...
async function recordPnlForRedeemedCondition(conditionId: string, balanceAfterRedeem?: number): Promise<void> {
    ...
    const slug = row.slug || key.replace(/^(copytrade|predictive-arb):/, "");
...
function pruneBotStateKeepNewest(keep: number): void {
    ...
    try {
        ...
        logger.error(`Failed to read predictive-arb-state.json for pruning: ${e instanceof Error ? e.message : String(e)}`);
        return;
    }
    ...
    try {
        ...
        logger.info(`Pruned predictive-arb-state.json removed=${removed} kept=${Object.keys(pruned).length}`);
    } catch (e) {
        logger.error(`Failed to write pruned predictive-arb-state.json: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
async function main() {
    ...
    const intervalMs = Number(getArgValue(args, "--interval-ms") ?? "200000"); // ~3–4× 5m windows
    ...
    const runOnce = async () => {
        ...
        try {
            ...
            // Keep state file small so bot startup stays fast.
    ...
    const waitMs =
        (align ? msUntilNextIntervalBoundary(config.predictiveArb.marketIntervalMinutes) : 0) + initialDelayMs;
    logger.info(
