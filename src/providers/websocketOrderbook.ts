...
export class WebSocketOrderBook {
    ...
    /**
     * Unsubscribe from tokens and remove callbacks + cached prices/labels.
     * Call this when rotating to a new market slug so stale IDs do not keep firing handlers.
     */
    detachTokenSubscriptions(assetIds: string[]): void {
        const ids = assetIds.filter((id) => typeof id === "string" && id.length > 0);
